...
#---------------- Blogs list/ individual blog/ newpost -----------------------
...
def validate_blog():
    ...
    if not title_error and not body_error:
        return render_template('single_blog.html', blog_title=blog_title, blog_body=blog_body, users=owner)
    else:
...
def main_blog():   
    #owner = User.query.filter_by(username=session['username']).first()
    if request.args.get('id'): #Individual blog
    ...
    elif request.args.get('user'): # single users blog
        
        
        return render_template('single_user.html')

    if not request.args.get('id'): #Blog users list      
        ...
        return render_template('blog.html', blog_titles=blog_titles, page_title='Blogz', 
            blog_bodyz=blog_bodyz)#not working
...
@app.before_request        
def require_login():
    allowed_routes = ['login', 'signup', 'main_blog', 'single_user', 'index']
...
@app.route('/signup', methods=['POST', 'GET'])#signup route
def signup():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
